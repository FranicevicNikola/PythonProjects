...
driver = Chrome('/Volumes/Seagate HDD/Nikola/chromedriver')
...
def video():
    ...
    driver.find_element(
        By.XPATH, '/html/body/div[4]/div/div/div/div/div[1]/div[2]/div[1]/div/div[1]/a/span')  # ocjeni button


def findopen_links():
    number_of_videos_in_playlist = 0
    driver.execute_script(
        'window.open("https://www.tonimilun.hr/gradivo/vedska-matematika-korjenovanje/","_blank");')
    driver.switch_to.window(driver.window_handles[1])
    sleep(5)
    playlist = driver.find_elements(By.CLASS_NAME, 'slide.ignore')
    for url in playlist:
        number_of_videos_in_playlist += 1
        driver.switch_to.window(driver.window_handles[1])
        sleep(3)
        driver.execute_script(
        ...
        video()
        sleep(3)
    print(number_of_videos_in_playlist)
